		...
		if (result.hasErrors()) {
			...
			if (user != null) {
				if (user.getRole().equals("ROLE_USER")) {
					...
					mv.addObject("ShowCarousel", "True");
					session.setAttribute("UserLoginSuccessMessage", "Welcome");
					log.debug("LoginController= Ending of the Method checklogin1()");
					...
					session.setAttribute("AdminSuccessMessage", "You are Logged in Admin");
					log.debug("LoginController= Ending of the Method checklogin2()");
				...
				mv.addObject("UserLoginErrorMessage", "Invalid Credentials");
				log.debug("LoginController= Ending of the Method checklogin4()");
